...
export function TimeSeriesChart({
  ...
  const domain = useMemo((): [number, number] => {
    ...
    return [Math.max(0, minVal - padding), yUnit === "%" ? Math.min(maxVal + padding, 100) : maxVal + padding];
  }, [chartData, yUnit]);
  ...
  return (
    <ResponsiveContainer width="100%" height={mobile ? 240 : 338} minWidth={0} minHeight={1}>
      <BarChart data={chartData} margin={mobile ? { top: 8, right: 4, left: 0, bottom: 0 } : { top: 10, right: 10, left: 10, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="var(--color-dash-border-soft)" strokeOpacity={0.6} vertical={false} horizontal />
        <XAxis
          dataKey="date"
          ...
          content={({ active, payload, label: tooltipLabel }) => {
            ...
            return (
              <div className="rounded-md border border-[#141414] bg-gradient-to-b from-[#434343] to-[#232323] px-3 py-2 shadow-[0px_2px_4px_rgba(0,0,0,0.18),inset_0px_1px_0px_rgba(255,255,255,0.18)]">
                <p className="text-[10px] leading-4 text-white/60">{tooltipLabel}</p>
