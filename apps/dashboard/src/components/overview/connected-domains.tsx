...
export function ConnectedDomains({ activeDomains = 0 }: { activeDomains?: number }) {
    ...
    <div className="mb-8 flex rounded-[4px] border-[0.5px] border-dash-border py-2">
      ...
      <div className="flex flex-1 flex-col gap-3.5 px-3.5 py-3.5">
        <div className="flex flex-col gap-2">
          <h2 className="text-base font-medium leading-5 tracking-[-0.03px] text-dash-text-strong">Connected domains</h2>
      ...
      <div className="hidden h-[122px] w-[169px] shrink-0 flex-col items-center justify-center border-l-[0.5px] border-dash-border pl-3.5 pr-2 sm:flex">
        <div className="flex flex-col gap-px">
          <span className="text-[56px] font-light leading-none tracking-[-0.09px] text-dash-text-strong">{activeDomains}</span>
          <Link to={withWorkspaceQuery({ pathname: "/domains", searchStr }) as any} className="group flex items-center gap-2">
            <span className="text-sm font-light leading-[1.3] text-dash-text-faded transition-colors group-hover:text-dash-text-strong">
              Active domains
            </span>
            <motion.span whileTap={{ x: 2, y: -2 }} transition={{ type: "spring", stiffness: 400, damping: 15 }}>
