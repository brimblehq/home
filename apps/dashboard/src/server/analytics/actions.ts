...
import type { AnalyticsGetInput, AnalyticsPayload, AnalyticsRecord } from "@/backend/analytics";
...
export type EnableAnalyticsResult = { ok: true; data: AnalyticsRecord } | { ok: false; planLocked: boolean; message: string };

export const enableAnalyticsServerFn = createServerFn({ method: "POST" }).handler(async ({ data }): Promise<EnableAnalyticsResult> => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Couldn't enable analytics right now. Please try again.";
    return { ok: false, planLocked: false, message };
  }
});

export const disableAnalyticsServerFn = createServerFn({ method: "POST" }).handler(async ({ data }) => {
  const payload = data as { projectId?: string } | undefined;
  const projectId = payload?.projectId?.trim();
...
export const getAnalyticsServerFn = createServerFn({ method: "GET" }).handler(async ({ data }): Promise<AnalyticsLoadResult> => {
  ...
  try {
    const result = await withTokenRefresh((api) =>
      api.analytics.get({
        ...
        type: payload.type,
        host: typeof payload.host === "string" && payload.host.trim().length > 0 ? payload.host.trim() : undefined,
      }),
    );
    ...
    if (error instanceof BackendApiError) {
      ...
      const raw = (error.message ?? "").toLowerCase();
      if (error.status === 400 && raw.includes("not available for your subscription")) {
        return {
          state: "plan-locked",
          message: "Web analytics is part of a paid plan. Upgrade to enable it.",
        };
    ...
    const message = error instanceof Error ? error.message : "We couldn't load analytics right now.";
