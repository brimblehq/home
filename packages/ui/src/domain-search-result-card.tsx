...
const variantClasses = {
  dashboard: {
    root:
      "flex items-center justify-between rounded-[4px] border-[0.5px] border-dash-border px-3.5 py-3 text-left transition-colors",
    ...
    taken:
      "rounded-full bg-dash-bg-elevated px-2.5 py-0.5 text-xs font-medium text-dash-text-faded",
  },
  web: {
    root:
      "flex items-center justify-between rounded-lg border border-[rgba(152,157,164,0.3)] bg-brimble-surface/90 px-3.5 py-3 text-left transition-colors dark:border-white/10 dark:bg-[#1e2023]",
    ...
    taken:
      "rounded-full bg-brimble-air-gray px-2.5 py-0.5 text-xs font-medium text-brimble-black/55 dark:bg-white/10 dark:text-white/65",
  },
} as const
...
export function DomainSearchResultCard({
    ...
    <>
      <span className={styles.text}>
        ...
        {isExactMatch && result.available && (
          <CheckCircle2 size={15} className={styles.exact} />
        )}
      </span>
      {result.available ? (
        <span className={styles.price}>
          {result.price === null ? "Available" : formatUsd(result.price)}
        </span>
      ) : (
  ...
  if (!interactive) {
    return (
      <div className={cn(styles.root, !result.available && styles.unavailable, className)}>
        {content}
      </div>
    )
  }
  ...
  return (
    <button
      ...
      className={cn(
        styles.root,
        result.available ? styles.hover : styles.unavailable,
        result.available && "cursor-pointer",
        className
      )}
    >
      {content}
